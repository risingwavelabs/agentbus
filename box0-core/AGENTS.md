...
- Always test changes before committing. Run `cargo test` at minimum (unit + integration tests). For user-facing features, run `tests/e2e.sh` which requires Codex or Codex.
- After code changes, always update README.md, AGENTS.md, docs/, and the skill content in config.rs if affected. This is critical.
...
- Workers support multiple runtimes: `auto` (default), `Codex`, or `codex`.
  - `auto` prefers Codex if installed, falls back to Codex.
  - Set per-worker via `--runtime Codex` or `--runtime codex`.
- Daemon spawns the runtime CLI in the worker's directory.
  - Codex: `Codex --print --output-format json --system-prompt "<instructions>"`, task piped via stdin.
...
- Session IDs are tracked per thread for multi-turn conversations (`--resume`, Codex only). Codex does not support session resume.
- Multi-turn: `b0 delegate --thread <id>` sends "answer" message, daemon resumes Codex session.
...
- Workers use the machine's local Codex/Codex authentication. No credential forwarding.
...
- `b0 skill install Codex` writes `~/.Codex/skills/b0/SKILL.md` (directory format, not plain file).
...
- API integration tests in `tests/api.rs` (15 tests). Start a real Axum server per test with temp DB, test via HTTP client. No Codex/Codex needed. Run with `cargo test`.
- E2e script in `tests/e2e.sh`. Requires Codex or Codex installed. Starts real server, runs CLI commands, verifies results. Run manually before releases.
